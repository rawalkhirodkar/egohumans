...
from mmpose.apis import (inference_top_down_center_pose_model, init_pose_model,
...
class CenterPoseModel:
    ...
    def get_poses2d(self, bboxes, image_name, camera_type='ego', camera_mode='rgb', camera=None, aria_humans=None,):
        ...
        ## add centers to the bboxes
        for i in range(len(bboxes)):
            bbox = bboxes[i]
            human_name = bbox['human_name']
            human = aria_humans[human_name]

            body_center, is_valid = camera.get_aria_location(point_3d=human.location)

            ## To do the camera center is not working ,use bbox center for now
            ## later train a model to predict the body center
            if is_valid:
                # bbox['body_center'] = body_center
                ## center of the bbox
                body_center = np.array([(bbox['bbox'][0] + bbox['bbox'])[2]/2, (bbox['bbox'][1] + bbox['bbox'][3])/2])
                bbox['body_center'] = body_center

            else:
                bbox['body_center'] = np.zeros(2)
            
        pose_results, returned_outputs = inference_top_down_center_pose_model(
                    self.pose_model,
                    image_name,
                    bboxes,
