...
def main(args):
    ...
    for batch_idx, batch in enumerate(tqdm(detection_data_loader)):
        ...
        detection_result = human_detector.detect_batch(norm_img, dim) # batch_id, min_x, min_y, max_x, max_y, det_conf, nms_conf, category_id
    ...
    if args.save_results:
        ...
        pred_vert_arr_save = np.array(pred_vert_arr)
        np.savez(result_filepath, imgname=img_path_list,
                 ...
                 detection_all=detection_all, pred_verts=pred_vert_arr_save, faces=smpl_model.faces)
