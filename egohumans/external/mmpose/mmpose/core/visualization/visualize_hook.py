...
class VisualizeSegHook(Hook):
    ...
    def after_val_iter(self, runner):
        ...
        segmentation = data_batch['segmentation']
        ...
        if len(image) > self.max_samples:
            ...
            segmentation = segmentation[:self.max_samples]
        ...
        save_batch_heatmaps(original_image, target, '{}_{}_hm_gt.jpg'.format(prefix, suffix), normalize=False, scale=self.scale)
        save_batch_heatmaps(original_image, output, '{}_{}_hm_pred.jpg'.format(prefix, suffix), normalize=False, scale=self.scale)

        save_batch_image_with_joints_and_seg(255*original_image, target, target_weight, segmentation, '{}_{}_gt.jpg'.format(prefix, suffix), scale=self.scale)
        save_batch_image_with_joints(255*original_image, output, torch.ones_like(target_weight), '{}_{}_pred.jpg'.format(prefix, suffix), scale=self.scale)
...
class VisualizeHook(Hook):
    ...
    def after_val_iter(self, runner):
        ...
        save_batch_image_with_joints(255*original_image, target, target_weight, '{}_{}_gt.jpg'.format(prefix, suffix), scale=self.scale)
        save_batch_image_with_joints(255*original_image, output, torch.ones_like(target_weight), '{}_{}_pred.jpg'.format(prefix, suffix), scale=self.scale)
...
class VisualizeCenterHook(Hook):
    ...
    def after_val_iter(self, runner):
        ...
        save_batch_image_with_joints_and_body_center(255*original_image, target, target_weight, body_center_heatmap, '{}_{}_gt.jpg'.format(prefix, suffix), scale=self.scale)
        save_batch_image_with_joints(255*original_image, output, torch.ones_like(target_weight), '{}_{}_pred.jpg'.format(prefix, suffix), scale=self.scale)
...
def save_batch_image_with_joints_and_seg(batch_image, batch_heatmaps, batch_target_weight, batch_segmentation, file_name, scale=4, nrow=8, padding=2):
    '''
    batch_image: [batch_size, channel, height, width]
    batch_joints: [batch_size, num_joints, 3],
    batch_joints_vis: [batch_size, num_joints, 1],
    ...
    for i in range(B):
        ...
        heatmap = batch_segmentation[i][0].mul(255)\
                                    .clamp(0, 255)\
                                    .byte()\
                                    .cpu().numpy()
        
        ## add dummy dimension to heatmap, it is a 256 x 192 image
        heatmap = np.expand_dims(heatmap, axis=2)
        masked_image = np.concatenate([heatmap]*3, axis=2)
        masked_image = 0.6*masked_image + 0.4*image

        ## where mask is 0, copy the original image
        masked_image = np.where(masked_image==0, image, masked_image)
...
def save_batch_image_with_joints_and_body_center(batch_image, batch_heatmaps, batch_target_weight, batch_body_center_heatmap, file_name, scale=4, nrow=8, padding=2):
    '''
    batch_image: [batch_size, channel, height, width]
    batch_joints: [batch_size, num_joints, 3],
    batch_joints_vis: [batch_size, num_joints, 1],
