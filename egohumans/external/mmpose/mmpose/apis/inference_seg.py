...
##------------------------------------------------------------------------------------
def _inference_single_seg_pose_model(model,
                                 img_or_path,
                                 bboxes,
                                 segmentations,
    ...
    batch_data = []
    for segmentation, bbox in zip(segmentations, bboxes):
...
def inference_top_down_seg_pose_model(model,
                                  img_or_path,
                                  person_results=None,
                                  bbox_thr=None,
                                  format='xywh',
    ...
    segmentations = np.array([box['segmentation'] for box in person_results])
    ...
    if bbox_thr is not None:
        ...
        segmentations = segmentations[valid_idx]
    ...
    with OutputHook(model, outputs=outputs, as_tensor=False) as h:
        ...
        poses, heatmap = _inference_single_seg_pose_model(
            model,
            img_or_path,
            bboxes_xywh,
            segmentations,
