...
class TopDownMpii2CocoDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    def _get_db(self):
        ...
        for a in anno:
            ...
            if not self.test_mode:
                joints = np.array(a['joints']) ## 16 joints for mpii
                joints_vis = np.array(a['joints_vis'])

                ## mpii dataset to coco dataset
                ## copy info from joints (16 dim) to joints_3d (17 dim) using MPII_KEYPOINT_INFO and COCO_KEYPOINT_INFO

                for mpii_joint_index in range(len(MPII_KEYPOINT_ORDER)):
                    coco_joint_index = MPII_TO_COCO_INDICES[mpii_joint_index]
                    if coco_joint_index is not None:
                        joints_3d[coco_joint_index, 0:2] = joints[mpii_joint_index, 0:2] - 1 ## make it 0-based index

                        joints_3d_visible[coco_joint_index, :2] = joints_vis[mpii_joint_index, None]
