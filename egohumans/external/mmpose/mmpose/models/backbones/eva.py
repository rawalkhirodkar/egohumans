...
class Eva(BaseBackbone):
    ...
    def forward_features(self, x):
        ...
        if self.pos_embed is not None:
            ...
            ## multi-scale testing
            if self.training == False and self.pos_embed.shape[1] - 1 != x.shape[1]:

                ## interpolate position embedding to the same size as x
                token_grid_height = self.patch_embed.grid_size[0] ## 32
                token_grid_width = self.patch_embed.grid_size[1] ## 24
                ...
                pos_tokens = pos_tokens.reshape(-1, token_grid_height, token_grid_width, embedding_size).permute(0, 3, 1, 2)
                pos_tokens = torch.nn.functional.interpolate(pos_tokens, size=(Hp, Wp), mode='bicubic', align_corners=False)
        ...
        ## interpolate rotation position embedding to the same size as x, multi-scale testing
        if self.training == False and rot_pos_embed is not None and rot_pos_embed.shape[0] != x.shape[1] - 1:
        ...
        xp = x.permute(0, 2, 1).reshape(B, -1, Hp, Wp).contiguous() ## 1024 x 32 x 24 for eva02, large, 3 x 448 x 336
