...
use_gt_bbox = True ## always use gt bbox for validation
# use_gt_bbox = False
...
model = dict(
    type='JudgeTopDown',
    judge_keypoint_idx=judge_keypoint_idx, ## nose
    jitter_prob=0.9,
    pretrained=None,
    pose_model_dict=pose_model_dict,
    pose_model_checkpoint=pose_model_checkpoint,
    backbone=dict(
        type='EvaJudge',
        img_size=model_image_resolution,
        patch_size=14,
...
val_pipeline = [
    ...
    dict(
        type='Collect',
        keys=['img', 'target', 'target_weight'], ## make sure the val is alway in gtbbox mode
        meta_keys=[
...
custom_hooks = [
    dict(type='JudgeVisualizeHook', vis_every_iters=vis_every_iters, max_samples=16, scale=3.5,),
]
