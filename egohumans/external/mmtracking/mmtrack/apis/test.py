...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        ...
        if show or out_dir:
            ...
            filename = img_meta['ori_filename'].split('/')
            is_custom = False
            thickness = 1
            font_scale = 0.5

            if 'ego' in filename or 'exo' in filename:
                is_custom = True
            ...
            if out_dir:
                ...
                if is_custom == True:
                    index = filename.index('ego') if 'ego' in filename else filename.index('exo')
                    temp = '/'.join(filename[index-2:]).replace('/images', '')
                    out_file = osp.join(out_dir, temp)
                    thickness = 3
                    font_scale = 0.5
            ...
            need_write_video = (
                ...
                or i == len(dataset))
            if out_dir and need_write_video and is_custom == False:
