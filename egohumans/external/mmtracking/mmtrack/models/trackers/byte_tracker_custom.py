...
from .base_tracker_custom import BaseTrackerCustom


@TRACKERS.register_module()
class ByteTrackerCustom(BaseTrackerCustom):
    ...
    def __init__(self,
        ...
        self.l2_dist = torch.nn.PairwiseDistance(p=2)
        return
    ...
    def init_track(self, id, obj):
        ...
        root_3d = self.tracks[id].roots_3d[-1]
        assert bbox.ndim == 2 and bbox.shape[0] == 1
        bbox = bbox.squeeze(0).cpu().numpy()
        root_3d = root_3d.squeeze(0).cpu().numpy()
        self.tracks[id].mean, self.tracks[id].covariance, self.tracks[id].mean_3d, self.tracks[id].covariance_3d = self.kf.initiate(
            bbox, root_3d)
    ...
    def update_track(self, id, obj):
        ...
        # if np.isnan(self.tracks[id].mean).sum() > 0:
        #     import pdb; pdb.set_trace()
        #     temp = 1

        self.tracks[id].mean, self.tracks[id].covariance, self.tracks[id].mean_3d, self.tracks[id].covariance_3d = self.kf.update(
            self.tracks[id].mean, self.tracks[id].covariance, bbox, self.tracks[id].mean_3d, self.tracks[id].covariance_3d, root_3d)
    ...
    def compute_euclidean_distance(self, roots_3d_a, roots_3d_b):
        distances_3d = torch.zeros((len(roots_3d_a), len(roots_3d_b))).to(roots_3d_a)

        for i in range(len(roots_3d_a)):
            for j in range(len(roots_3d_b)):
                distances_3d[i, j] = self.l2_dist(roots_3d_a[i], roots_3d_b[j])

        ## normalize by max distance
        if len(roots_3d_a) != 0 and len(roots_3d_b) != 0:
            distances_3d /= distances_3d.max() + 1e-6
    ...
    def assign_ids(self,
        ...
        ##---root_3d----
        track_roots_3d = np.zeros((0, 3))
        for id in ids:
            track_roots_3d = np.concatenate(
                (track_roots_3d, self.tracks[id].mean_3d[:3][None]), axis=0)
        track_roots_3d = torch.from_numpy(track_roots_3d).to(det_roots_3d)

        # compute distance
        ious = bbox_overlaps(track_bboxes, det_bboxes[:, :4])

        ## compute root_3d distances
        distances_3d = self.compute_euclidean_distance(track_roots_3d, det_roots_3d)
        ...
        # dists = (1 - ious + cate_cost + weight_3d*distances_3d).cpu().numpy()
        dists = ( self.kf.weight_2d*(1 - ious) + cate_cost + self.kf.weight_3d*distances_3d).cpu().numpy()
        ...
        if dists.size > 0:
            ...
            cost, row, col = lap.lapjv(
                dists, extend_cost=True, cost_limit=np.inf)
        else:
    ...
    def track(self,
        ...
        """
        assert(len(bboxes) == len(roots_3d))
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            first_det_roots_3d = roots_3d[first_det_inds]
            ...
            second_det_roots_3d = roots_3d[second_det_inds]
            ...
            for id in self.confirmed_ids:
                ...
                if self.tracks[id].frame_ids[-1] != frame_id - 1:
                    self.tracks[id].mean[7] = 0 ## absolute height velocity is set to zero
                    self.tracks[id].mean_3d[5] = 0 ## TO DO: is this correct?

                (self.tracks[id].mean,
                 self.tracks[id].covariance, self.tracks[id].mean_3d,
                 self.tracks[id].covariance_3d) = self.kf.predict(
                     self.tracks[id].mean, self.tracks[id].covariance, self.tracks[id].mean_3d, self.tracks[id].covariance_3d)

            # 2. first match
            first_match_track_inds, first_match_det_inds = self.assign_ids(
                self.confirmed_ids, first_det_bboxes, first_det_labels, first_det_roots_3d,
            ...
            first_match_det_roots_3d = first_det_roots_3d[valid]
            ...
            first_unmatch_det_roots_3d = first_det_roots_3d[~valid]
            ...
            (tentative_match_track_inds,
             tentative_match_det_inds) = self.assign_ids(
                 ...
                 first_unmatch_det_labels, first_unmatch_det_roots_3d, self.weight_iou_with_det_scores,
            ...
            second_match_track_inds, second_match_det_inds = self.assign_ids(
                first_unmatch_track_ids, second_det_bboxes, second_det_labels, second_det_roots_3d,
            ...
            roots_3d = torch.cat(
                (first_match_det_roots_3d, first_unmatch_det_roots_3d), dim=0)
            roots_3d = torch.cat((roots_3d, second_det_roots_3d[valid]), dim=0)
        ...
        self.update(ids=ids, bboxes=bboxes, labels=labels, frame_ids=frame_id, roots_3d=roots_3d)
