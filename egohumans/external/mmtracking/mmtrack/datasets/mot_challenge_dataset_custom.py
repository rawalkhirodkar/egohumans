...
class MOTChallengeDatasetCustom(CocoVideoDataset):
    ...
    def __init__(self,
        ...
        ## egoformer predictions on the yolox bounding box
        if egoformer_yolox_file is not None:
            self.egoformer_yolox_detections = mmcv.load(egoformer_yolox_file)
        else:
            self.egoformer_yolox_detections = None

        # ###----filter out images without any detections for egoformer and yolox----
        # data_infos_ = []

        # print('filtering images with no detections')
        # filter_images = []
        # for data_info in self.data_infos:
        #     if data_info['filename'] in self.detections.keys():
        #         data_infos_.append(data_info)
        #     else:
        #         filter_images.append(data_info['filename'])

        # print('images filtered {}'.format(len(filter_images)))
        # print(filter_images)
        # self.data_infos = data_infos_
    ...
    def load_detections(self, detection_file=None):
        ...
        if detection_file is not None:
            ...
            if isinstance(detections, dict):
                ...
                if 'roots_3d' in detections:
                    self.roots_3d = detections['roots_3d']
    ...
    def prepare_results(self, img_info):
        ...
        if self.detections is not None:
            ...
            if self.roots_3d is not None:
                results['roots_3d'] = self.roots_3d[indice]

        if self.egoformer_yolox_detections is not None:
            indice = img_info['file_name']

            if indice in self.egoformer_yolox_detections['det_bboxes'].keys():
                results['egoformer_yolox_detections'] = self.egoformer_yolox_detections['det_bboxes'][indice]
                results['egoformer_yolox_roots_3d'] = self.egoformer_yolox_detections['roots_3d'][indice]
            else:
                results['egoformer_yolox_detections'] = [np.zeros((1, 5))]
                results['egoformer_yolox_roots_3d'] = [np.zeros((1, 3))]
    ...
    def evaluate(self,
        ...
        tracking_file_name = self.ann_file.split('/')[-1]
        tracking_dir = '/'.join(self.ann_file.split('/')[:-1])

        ### if custom data, evaluatate using this
        if 'exo' in tracking_file_name or 'ego_rgb' in tracking_file_name or 'ego_slam' in tracking_file_name:
            return CocoVideoDataset.evaluate(self, results, metrics, logger, resfile_path)
        ...
        if 'track' in metrics:
            ...
            for name in names:
                ...
                elif 'exo' in tracking_file_name:
                    gt_file = osp.join(tracking_dir, 'gt_exo.txt')
                    custom_gt_dir_path = osp.join(local_dir.name, name, 'coco_track')
                    os.makedirs(custom_gt_dir_path)

                elif 'ego_rgb' in tracking_file_name:
                    gt_file = osp.join(tracking_dir, 'gt_ego_rgb.txt')
                    custom_gt_dir_path = osp.join(local_dir.name, name, 'coco_track')
                    os.makedirs(custom_gt_dir_path)

                elif 'ego_slam' in tracking_file_name:
                    gt_file = osp.join(tracking_dir, 'gt_ego_slam.txt')
                    custom_gt_dir_path = osp.join(local_dir.name, name, 'coco_track')
                    os.makedirs(custom_gt_dir_path)
