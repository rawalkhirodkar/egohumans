...
def run(input_path, output_path, model_path, model_type="large", optimize=True):
    ...
    aria_names = [val for val in sorted(os.listdir(os.path.join(input_dir, 'ego'))) if val.startswith('aria')]

    for aria_name in aria_names:
        input_path = os.path.join(input_dir, 'ego', aria_name, 'images', 'undistorted_rgb')
        output_path = os.path.join(output_dir, 'processed_data', 'depth', aria_name)
