...
class CrouchRegLoss(nn.Module):
    ...
    def forward(self,
        ...
        body_pose_reshape = body_pose.reshape(batch_size, -1, 3) ## B x 23 x 3

        ## SMPL joint order: https://github.com/facebookresearch/frankmocap/blob/main/docs/joint_order.md
        ## for 23 joints, idx 12 is L_collar, idx 13 is R_collar
        ## spin03, spin02, left collar, right collar
        crouch_prior_loss = loss_weight * ( \
                        body_pose_reshape[:, 8]**2 + \
                        body_pose_reshape[:, 5]**2 + \
                        body_pose_reshape[:, 2]**2 + \
                        body_pose_reshape[:, 12]**2 + \
                        body_pose_reshape[:, 13]**2 
                        )

        if reduction == 'mean':
            crouch_prior_loss = crouch_prior_loss.mean()
        elif reduction == 'sum':
            crouch_prior_loss = crouch_prior_loss.sum()

        return crouch_prior_loss
...
class SmoothJointLoss(nn.Module):
    ...
    def forward(self,
        ...
        rot_6d_diff = rot_6d[1:] - rot_6d[:-1] ## this considers temporal smoothing, does not works for batch size of 1!
